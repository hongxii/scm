...
<body onLoad="init()">
<form action="/sell/edit" method="post">
...
<form action="sell/edit" name="mainFrm">
    <table width="100%"  border="0" cellpadding="0" cellspacing="0">
        <tr>
    ...
    <table id="headTable" width="100%"  border="0" align="center" class="a1">
        <tr align="justify">
            <td>销售单编号</td>
            <td><input type="text" value="${somain.soID}" size="15" name="soID" readonly/>
                <span class="requred_symbol">*</span></td>
            <td>创建时间</td>
            <td><input type="text" size="15" name="createTime" id="time" value="${somain.createTime} " >
                <span class="requred_symbol">*</span></td>
            <td>客户名称</td>
            <td><input type="text" size="15" onBlur="test()" name="customerName" id="customerName" value="${somain.customerCode}"/>
                <span class='LL'><image src='../images/selectDate.gif' onclick="showcDiv()" <%--onClick='choiceSpxx("\" + (oRow.rowIndex - 1) + "\")'--%>></span>
        ...
        <tr align="justify">
            <td>附加费用</td>
            <td><input type="text" size="15" name="tipFee" value="${somain.tipFee}" />
                <span class="requred_symbol">*</span></td>
            <td>产品总价</td>
            <td>
                <input type="text" size="15" name="productTotal" id="total" value="${somain.productTotal}" readonly/>
            </td>
            <td>付款方式</td>
            <td>
                <select name="PayType" >
            ...
            <td>最低预付金额</td>
            <td><input name="prePayFee" type="text" size="15" value="${somain.prePayFee}" /></td>
        </tr>
        <tr>
            <td class="title2"></td>
        </tr>
    ...
    <table width="100%"  border="0" align="center" cellspacing="1" id="detailTable" onclick="setAll()">
        ...
        <c:forEach items="${soitems}" var="si" varStatus="th">
            <tr onClick="" onMouseOver="OMO1(event)" onDblClick="choice(1)" align="center">
                <td>${th.index+1}</td>
                <td><input type='text'  name='spbm' size='10' readonly value="${si.productCode}"> <span class='LL'><image src='../images/selectDate.gif' onClick="choiceSpxx('${th.index}')"/></span></td>
                <td><input type='text' name='spmc' size='15' value="${si.productCode}" readonly></td>
                <td><input type='number' name='ytsl' size='10' value='${si.num}'></td>
                <td><input type='text' name='sptj' size='10' value='${si.unitName}'  readonly></td>
                <td><input type='text' name='spzl' size='10' value='${si.unitPrice}' readonly></td>
                <td><input type='text' name='stsl' size='10' value='${si.itemPrice}' readonly></td>
                <td><image src="../images/delete.gif" class="LL" onclick="delItem('${th.index+1}')"/></td>
            </tr>
        </c:forEach>
    </table>
