...
function choiceAnonymous() {
	...
	for (i = 1; i < len - 1; i++) {
		if (spxxTable.rows[i].cells[0].innerHTML == "\u221a") {
			...
			//选中商品编号和表格原有编号对比若存在不 setvalues();
            var arr = document.getElementsByName("spbm");
            var tf = false;
            for(var t=0;t<arr.length;t++){
            	if(returnValue[0]==arr[t].value){
                    tf = true;
                    break;
				}
			}
...
function addItem() {
	...
	oCell.innerHTML = "<input type='number' name='ytsl' size='10' value='0'>";
	oCell = oRow.insertCell(4);
	oCell.innerHTML = "<input type='text' name='sptj' size='10' value='0' readonly>";
	oCell = oRow.insertCell(5);
	oCell.innerHTML = "<input type='text' name='spzl' size='10' value='0' readonly>";
	oCell = oRow.insertCell(6);
	oCell.innerHTML = "<input type='text' name='stsl' size='10' value='0' readonly>";
